...
public class SucursalController {
	...
	private static final String QUERY_CRUD_SUCURSALES= "SELECT * FROM special.crud_sucursal(?,?,?)";
	 ...
	 */
	@PostMapping(path = "/sucursales",consumes = JSON_PROD_CONS,produces = JSON_PROD_CONS)
	public String crudAlamcen(@AuthenticationPrincipal Jwt jwt, @RequestBody RequestTenant request) {
		...
		return service.execQueryDinamicoTenant(request, QUERY_CRUD_SUCURSALES);
	}
